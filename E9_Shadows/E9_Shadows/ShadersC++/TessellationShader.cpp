...
void TessellationShader::initShader(const wchar_t* vsFilename, const wchar_t* psFilename)
{
	...
	tessellationBufferDesc.ByteWidth = sizeof(MatrixBufferType);
...
void TessellationShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, float tessellationFactors[4], Camera* cam, float tessellationRange[2], ID3D11ShaderResourceView* heightTex)
{	
	HRESULT result;
	...
	deviceContext->HSSetShaderResources(0, 1, &heightTex);
	deviceContext->HSSetSamplers(0, 1, &heightSampleState);

	// Set shader height texture resource in the domain shader.
	deviceContext->DSSetShaderResources(0, 1, &heightTex);
	deviceContext->DSSetSamplers(0, 1, &heightSampleState);

	deviceContext->PSSetShaderResources(0, 1, &heightTex);
	deviceContext->PSSetSamplers(0, 1, &heightSampleState);
}
