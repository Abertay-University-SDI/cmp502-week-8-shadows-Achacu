...
void TessellationPlane::initBuffers(ID3D11Device* device)
{
	...
	indexCount = resolution * resolution * 4;
	...
	for (j = 0; j < resolution+1; j++)
	{
		...
		{
			vertices[index].position = XMFLOAT3((float)i, 0.0f, (float)j);
			vertices[index].normal = XMFLOAT3(0.0f,1.0f,0.0f);
			vertices[index].texture = XMFLOAT2(i*increment, j*increment);
			index++;
		}
	}
	...
	for (j = 0; j < resolution; j++)
	{
		...
		{
			indices[index++] = i + j*(resolution+1); //bottom left
			indices[index++] = (i+1) + j*(resolution+1); //bottom right
			indices[index++] = (i+1) + (j+1)*(resolution+1); //top right			
			indices[index++] = i + (j+1)*(resolution+1); //top left
		}
	}
	//for (j = 0; j < (resolution - 1); j++)
	//{
